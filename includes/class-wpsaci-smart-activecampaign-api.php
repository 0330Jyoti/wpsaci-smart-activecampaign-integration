<?php
class WPSACI_Smart_ActiveCampaign_API {
    ...
    function __construct() {
        ...
        $client_id                  = esc_attr($wpsaci_smart_activecampaign_settings['client_id']);
        $client_secret              = esc_attr($wpsaci_smart_activecampaign_settings['client_secret']);
        $wpsaci_smart_activecampaign_data_center  = esc_attr($wpsaci_smart_activecampaign_settings['data_center']);
        ...
        $this->url              = $wpsaci_smart_activecampaign_data_center;
        $this->client_id        = $client_id;
        $this->client_secret    = $client_secret;
        $this->token            = get_option( 'wpsaci_smart_activecampaign' );

        // Get any existing copy of our transient data
        if ( false === ( $wpsaci_smart_activecampaign_expire = get_transient( 'wpsaci_smart_activecampaign_expire' ) ) ) {
    ...
    function loadAPIFiles(){
        require_once WPSACI_PLUGIN_PATH . 'includes/class.getListofModules.php';
        require_once WPSACI_PLUGIN_PATH . 'includes/class.getFieldsMetaData.php';
    }
    ...
    function getRefreshToken( $token ) {
        ...
        if ( isset( $response->access_token ) ) {
            ...
            $wpsaci_smart_activecampaign_expire = 'Expire_Management';
            set_transient( 'wpsaci_smart_activecampaign_expire', $wpsaci_smart_activecampaign_expire, 3500 );
            update_option( 'wpsaci_smart_activecampaign', $token );
        }
    ...
    function manageToken( $token ){
        $old_token = get_option( 'wpsaci_smart_activecampaign' );
    ...
    function addRecord( $module, $data ) {
        ...
        if ( isset( $response->data[0]->status ) && $response->data[0]->status == 'error' ) {
            ...
            file_put_contents( WPSACI_PLUGIN_PATH.'debug.log', $log, FILE_APPEND );
        }
